...
import styles from '../FeedList/Reaction.module.css';
import { useState } from 'react';

export function Reaction({ id, like, dislike }) {
  const [clickLike, setClickLike] = useState(like);
  const [clickDislike, setClickDislike] = useState(dislike);
  const [likePost, setLikePost] = useState(true);
  const [dislikePost, setDislikePost] = useState(true);

  const onClickLike = () => {
    if (clickLike === like + 1) setClickLike(like);
    else if (likePost) {
      setClickLike(like + 1);
      setLikePost(false);
      postReaction(id, 'like');
    } else setClickLike(like + 1);
  };

  const onClickDislike = () => {
    if (clickDislike === dislike + 1) setClickDislike(dislike);
    else if (dislikePost) {
      setClickDislike(dislike + 1);
      setDislikePost(false);
      postReaction(id, 'dislike');
    } else setClickDislike(dislike + 1);
  };
  ...
  return (
    <div className={styles['btn-reaction']}>
      <button
        type="button"
        className={`${styles['btn']} ${styles['btn-like']} ${clickLike === like + 1 ? styles['active'] : ''}`}
        onClick={onClickLike}
      >
        <svg
          width="16"
        ...
        좋아요 {clickLike !== 0 && clickLike}
      </button>
      <button
        type="button"
        className={`${styles['btn']} ${styles['btn-dislike']} ${clickDislike === dislike + 1 ? styles['active'] : ''}`}
      ...
      >
        ...
        싫어요 {clickDislike !== 0 && clickDislike}
      </button>
    </div>
  );
}
